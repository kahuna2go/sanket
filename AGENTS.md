...
- To inspect what a live strategy actually did, do NOT use `docker compose logs` — strategy loggers (orb/smc/smoby) write only to `logs/<name>.log` inside the container via a `RotatingFileHandler` with `propagate=False`; they never reach stdout. `docker compose logs` shows only the launcher's aiohttp access log and unrelated scanner-noise tracebacks. SSH in (`ssh root@178.105.135.229`) and read `logs/<name>.log` directly. Lines are `[HH:MM:SS]` with no date, and the server clock is UTC — isolate one trading day with `awk '/New day reset \(YYYY-MM-DD\)/{f=1} /New day reset \(next-day\)/{f=0} f' logs/orb.log`. Cross-check trade outcomes in `data/trades.jsonl` (`grep '"strategy": "orb"' data/trades.jsonl`) — its `ts` field is UTC ISO and lines up with the log timestamps.
- `place_close_order()` on HIP-3 assets (e.g. `xyz:SP500`) can silently return `None` instead of raising: the Hyperliquid SDK's `Exchange.market_close()` matches the open position by exact string equality against `coin` from `user_state()`, but HIP-3 position records come back with the bare symbol (`SP500`), not the `xyz:`-prefixed name used everywhere else in this codebase — so the match never fires, the SDK function falls off the end and returns `None`, and no exception is raised. Confirmed live 2026-07-10: ORB's TP1 partial-close retried every 60s for ~3 hours ("Unexpected order response format: None") without ever closing. Any caller of `place_close_order` for a HIP-3 asset must check `extract_filled_size()` on the result before trusting it.
- A strategy subprocess can hang (stop logging entirely, no exception, no crash) without the launcher noticing — the process stays alive (`proc.returncode is None`) so `_is_running()` reports healthy. Don't infer "it's fine" from the process existing; check `logs/<name>.log` for a recent heartbeat (e.g. today's "New day reset" line) before trusting a strategy is actually progressing.
