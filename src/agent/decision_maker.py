...
class TradingAgent:
    ...
    async def _decide(self, context, assets, model=None, macro_context=None):
        ...
        system_prompt = (
            ...
            "IMPORTANT: ts=1 and ts=2 require an EXISTING thesis to weaken or break — they imply\n"
            "  a prior directional view that is now deteriorating. If there is no open position\n"
            "  AND bias=null / swing_count<2 / prerequisites unmet, the correct rating is ts=3\n"
            "  (no edge), NOT ts=1. 'No structure' ≠ 'structure broken'.\n"
