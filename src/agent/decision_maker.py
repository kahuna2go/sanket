...
class TradingAgent:
    ...
    def _decide(self, context, assets, model=None, macro_context=None):
        ...
        system_prompt = (
            ...
            "Use 1h Market Structure + 5m Value Area Bounce:\n"
            "  1h bias (all required before considering any entry):\n"
            "    - bias_1h.bias = 'bull' (long) or 'bear' (short)\n"
            "    - bias_1h.trend = HH_HL (long) or LH_LL (short)\n"
            "    - bias_1h.swing_count ≥ 2 (at least hh-hl-hh or ll-lh-ll confirmed)\n"
            ...
            "           (touched lower VA zone, closed back above VAL)\n"
            "    Short: 5m bar.high ≥ bias_1h.vah − 0.30×va_width  AND  bar.close < bias_1h.vah\n"
            "           (touched upper VA zone, closed back below VAH)\n"
            "    Volume filter: intraday.rvol ≥ 1.2 on the signal bar. If below, skip.\n"
            "    Confirmation: the following 5m bar must close above bias_1h.val (long) or\n"
            "                  below bias_1h.vah (short) before entry is valid.\n"
            "                  If bias_1h.bias changes before confirmation, cancel the pending entry.\n"
            "  SL placement:\n"
            "    Long:  bias_1h.val − 0.15×va_width\n"
            "    Short: bias_1h.vah + 0.15×va_width\n"
            "  TP and partial exit plan:\n"
            "    TP1 (50% of position): bias_1h.vah (long) or bias_1h.val (short)\n"
            "      When TP1 hit: close half the position, move SL to entry (breakeven)\n"
            "    TP2 (remaining 50%): bias_1h.tp_speculative_long (long) or bias_1h.tp_speculative_short (short)\n"
            "      Fibonacci 127.2% extension of the last 1h swing\n"
            "    Set tp_price = TP2 when placing the order. Encode TP1 level and partial-exit plan in exit_plan.\n"
            "  R:R check: compute (TP1 − entry) ÷ (entry − SL). Require ≥ 1.5:1. If below, do not open.\n"
            "  Exit / flip:\n"
            "    If 1h closes below bias_1h.val (long) or above bias_1h.vah (short): tighten SL to breakeven.\n"
            "    If bias_1h.bias flips to opposite direction: thesis broken — close position.\n"
